    ...
    <div x-data="{ scrolled: false, menuOpen: false }" x-init="window.addEventListener('scroll', () => { scrolled = window.scrollY > 32; })">
        ...
        <header :class="scrolled ? 'nav-shell scrolled' : 'nav-shell'" class="fixed inset-x-0 top-0 z-50 border-b border-[#d4a563]/25">
            <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
                <a href="<?php echo e(route('dashboard.tourist')); ?>" class="flex items-center gap-3">
                    <img src="<?php echo e(asset('images/tribaltours_icon.png')); ?>" alt="TrblTours Icon" class="h-10 w-10 rounded-full border border-[#d4a563]/60 bg-[#f6ecd8] object-cover shadow-md">
                ...
                <div class="relative" @click.outside="menuOpen = false">
                    <button @click="menuOpen = !menuOpen" class="inline-flex items-center gap-2 rounded-full border border-[#e8d2ab] bg-[#fff6e8] px-4 py-2 text-sm font-semibold text-[#5a3c2a] transition hover:border-[#d4a563] hover:bg-[#f7e8cf]">
                        <span class="flex h-8 w-8 items-center justify-center rounded-full bg-[#8f9d59] text-xs font-bold text-white">
                            <?php echo e(strtoupper(substr($currentUser->full_name ?? $currentUser->name ?? 'T', 0, 1))); ?>

                        </span>
                        <span class="hidden sm:inline"><?php echo e($currentUser->full_name ?? $currentUser->name ?? 'Tourist'); ?></span>
                    ...
                    <div x-show="menuOpen" x-transition class="absolute right-0 mt-2 w-64 overflow-hidden rounded-2xl border border-[#d4a563]/40 bg-[#fffaf3] py-2 shadow-2xl">
                        ...
                        <a href="<?php echo e(route('profile.edit')); ?>" class="block px-4 py-2.5 text-sm text-[#5a3c2a] transition hover:bg-[#f7ecd7]">Profile Dashboard (edit profile)</a>
        ...
        <section class="filter-shell sticky top-[72px] z-40 border-b border-[#d4a563]/25 bg-[#fffaf3]/95">
            <div class="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
                <form method="GET" action="<?php echo e(route('dashboard.tourist')); ?>" class="grid gap-4 md:grid-cols-3">
                    <div>
                        ...
                        <select id="location" name="location" class="w-full rounded-xl border border-[#d6bc93] bg-white text-sm text-[#5a3c2a] shadow-sm focus:border-[#8f9d59] focus:ring-[#8f9d59]/30">
                            ...
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $regions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $region): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($region); ?>" <?php if($location === $region): echo 'selected'; endif; ?>><?php echo e($region); ?></option>
                    ...
                    <div>
                        ...
                        <select id="sort_by" name="sort_by" class="w-full rounded-xl border border-[#d6bc93] bg-white text-sm text-[#5a3c2a] shadow-sm focus:border-[#8f9d59] focus:ring-[#8f9d59]/30">
                            <option value="latest" <?php if($sortBy === 'latest'): echo 'selected'; endif; ?>>Latest</option>
                            <option value="price_low_high" <?php if($sortBy === 'price_low_high'): echo 'selected'; endif; ?>>Price Low to High</option>
                            <option value="price_high_low" <?php if($sortBy === 'price_high_low'): echo 'selected'; endif; ?>>Price High to Low</option>
                        </select>
                    </div>
        ...
        <main class="mx-auto max-w-7xl px-4 pb-24 pt-8 sm:px-6 lg:px-8">
            <section class="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $feedItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <?php
                        $type = $item['type'] ?? null;
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($type === 'tour_listing' && $record): ?>
                        <article class="group flex h-full flex-col rounded-2xl border border-[#e2cda9] bg-white p-6 shadow-[0_14px_36px_-24px_rgba(58,39,26,0.55)] transition duration-300 hover:-translate-y-0.5 hover:border-[#d4a563] hover:shadow-[0_18px_44px_-22px_rgba(58,39,26,0.7)]">
                            ...
                            <p class="mt-3 text-sm text-[#6b4a34]">
                                Guide:
                                <span class="font-semibold text-[#4b3224]"><?php echo e($guide->full_name ?? $guide->name ?? 'Verified Guide'); ?></span>
                            </p>

                            <h2 class="mt-2 text-xl font-bold text-[#3e2a1f]"><?php echo e($record->title ?? $record->name ?? 'Untitled Tour'); ?></h2>
                            <p class="mt-2 text-sm text-[#73543e]">Location: <?php echo e($record->region ?? 'Region not specified'); ?></p>
                            <p class="mt-1 text-sm text-[#73543e]">Duration: <?php echo e($record->duration_label ?? 'Flexible duration'); ?></p>
                            <p class="mt-4 text-2xl font-extrabold text-[#3e2a1f]">PHP <?php echo e(number_format((float) ($record->price_per_person ?? $record->price ?? 0), 0)); ?></p>
                    ...
                    <?php elseif($type === 'request_post' && $record): ?>
                        <article class="flex h-full flex-col rounded-2xl border border-[#e2cda9] bg-white p-6 shadow-[0_14px_36px_-24px_rgba(58,39,26,0.55)] transition duration-300 hover:border-[#d4a563] hover:shadow-[0_18px_44px_-22px_rgba(58,39,26,0.68)]">
                            ...
                            <h2 class="mt-2 text-lg font-bold text-[#3e2a1f]"><?php echo e($record->tour?->title ?? 'Custom Tour Request'); ?></h2>
                            <p class="mt-1 text-sm text-[#73543e]">Location: <?php echo e($record->tour?->region ?? 'Region not specified'); ?></p>
                            <p class="mt-2 text-sm text-[#73543e]">Group Size: <?php echo e($record->group_size ?? 'N/A'); ?></p>
                            <p class="mt-2 text-xl font-bold text-[#3e2a1f]">PHP <?php echo e(number_format((float) ($record->total_price ?? 0), 0)); ?></p>
                            <div class="mt-6">
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(method_exists($posts, 'links')): ?>
                <div class="mt-8">
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($isTourist): ?>
            <div class="fixed bottom-6 right-6 z-50">
                <a href="<?php echo e(route('requests.store')); ?>"
