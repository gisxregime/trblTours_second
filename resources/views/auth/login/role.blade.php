...
<head>
    ...
    <title>{{ config('app.name', 'TrblTours') }} - Login Role</title>
...
<body>
    ...
    <main class="min-h-screen flex items-center justify-center px-4 py-8">
        <section class="w-full max-w-xl rounded-2xl border border-[#d4a563]/30 bg-[#fffbf4] p-8 shadow-[0_18px_40px_-20px_rgba(122,85,50,0.45)]">
            ...
            <h1 class="mt-4 text-3xl font-semibold text-[#23170f]">Choose Your Login Role</h1>
            <p class="mt-2 text-sm text-[#6f5d52]">Step 2 of 3: email {{ $draft->email }} is verified.</p>
            ...
            <form method="POST" action="{{ route('login.role.store', ['token' => $draft->token]) }}" class="mt-6 space-y-4">
                @csrf

                <label class="flex items-start gap-3 rounded-xl border border-[#d4a563]/30 p-4 hover:bg-[#fff8eb]">
                    <input type="radio" name="role" value="tourist" class="mt-1" {{ old('role', 'tourist') === 'tourist' ? 'checked' : '' }}>
                    <span>
                        <span class="block font-semibold">Tourist</span>
                        <span class="block text-sm text-[#6f5d52]">Login to book and manage tours.</span>
                    </span>
                </label>

                <label class="flex items-start gap-3 rounded-xl border border-[#d4a563]/30 p-4 hover:bg-[#fff8eb]">
                    <input type="radio" name="role" value="tour_guide" class="mt-1" {{ old('role') === 'tour_guide' ? 'checked' : '' }}>
                    <span>
                        <span class="block font-semibold">Tour Guide</span>
                        <span class="block text-sm text-[#6f5d52]">Login to manage your guide dashboard.</span>
                    </span>
                </label>
