...
<body>
    ...
    <main class="min-h-screen flex items-center justify-center px-4 py-8">
        <section class="panel">
            <a href="{{ url('/') }}" class="inline-flex items-center gap-2 rounded-full border border-[#c9b59f] bg-[#f4efe6] px-3 py-1.5 text-[13px] font-semibold text-[#3f2d1f] shadow-sm transition hover:bg-[#ece1d0] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8b4e1c]/30">
            ...
            @if ($step === 1)
                <form method="POST" action="{{ route('signup.email.store') }}" class="space-y-4">
                    @csrf
            ...
            @elseif ($step === 2 && $draft)
                <form method="POST" action="{{ route('signup.otp.store', ['token' => $draft->token]) }}" class="space-y-4">
                    @csrf
                ...
                <form method="POST" action="{{ route('signup.otp.resend', ['token' => $draft->token]) }}" class="mt-3">
                    @csrf
            ...
            @elseif ($step === 3 && $draft)
                <form method="POST" action="{{ route('signup.role.store', ['token' => $draft->token]) }}" class="space-y-4">
                    @csrf
            ...
            @elseif ($step === 4 && $draft)
                <form method="POST" action="{{ route('signup.details.store', ['token' => $draft->token]) }}" class="space-y-4">
                    @csrf

                    <p class="border-b border-[#d4c5b2] pb-2 text-[11px] font-semibold uppercase tracking-[0.09em] text-[#604c3a]">Profile Details</p>
                    <p class="text-sm text-[#6f5b46]">Verified email: {{ $draft->email }} | Role: {{ $isGuide ? 'Tour Guide' : 'Tourist' }}</p>
                    ...
                    <div>
                        ...
                        <input id="full_name" name="full_name" type="text" value="{{ old('full_name') }}" required class="w-full rounded-lg border border-[#d8c9b5] bg-[#f4efe6] px-3 py-2.5 text-sm text-[#2f241a] outline-none transition focus:border-[#9a5f2a] focus:ring-0">
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input id="date_of_birth" name="date_of_birth" type="date" value="{{ old('date_of_birth') }}" required class="w-full rounded-lg border border-[#d8c9b5] bg-[#f4efe6] px-3 py-2.5 text-sm text-[#2f241a] outline-none transition focus:border-[#9a5f2a] focus:ring-0">
                        </div>
                        <div>
                            ...
                            <input id="phone_number" name="phone_number" type="text" value="{{ old('phone_number') }}" required class="w-full rounded-lg border border-[#d8c9b5] bg-[#f4efe6] px-3 py-2.5 text-sm text-[#2f241a] outline-none transition focus:border-[#9a5f2a] focus:ring-0">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="nationality" name="nationality" required class="w-full rounded-lg border border-[#d8c9b5] bg-[#f4efe6] px-3 py-2.5 text-sm text-[#2f241a] outline-none transition focus:border-[#9a5f2a] focus:ring-0">
                            @foreach ($nationalities as $nationality)
                                <option value="{{ $nationality }}" @selected(old('nationality', 'Filipino') === $nationality)>{{ $nationality }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    @if ($isGuide)
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input id="government_id_number" name="government_id_number" type="text" value="{{ old('government_id_number') }}" required class="w-full rounded-lg border border-[#d8c9b5] bg-[#f4efe6] px-3 py-2.5 text-sm text-[#2f241a] outline-none transition focus:border-[#9a5f2a] focus:ring-0">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <input id="years_of_experience" name="years_of_experience" type="number" min="0" value="{{ old('years_of_experience') }}" required class="w-full rounded-lg border border-[#d8c9b5] bg-[#f4efe6] px-3 py-2.5 text-sm text-[#2f241a] outline-none transition focus:border-[#9a5f2a] focus:ring-0">
                            </div>
                            <div>
                                ...
                                <input id="tour_guide_cert_number" name="tour_guide_cert_number" type="text" value="{{ old('tour_guide_cert_number') }}" class="w-full rounded-lg border border-[#d8c9b5] bg-[#f4efe6] px-3 py-2.5 text-sm text-[#2f241a] outline-none transition focus:border-[#9a5f2a] focus:ring-0">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="bio" name="bio" rows="4" required class="w-full rounded-lg border border-[#d8c9b5] bg-[#f4efe6] px-3 py-2.5 text-sm text-[#2f241a] outline-none transition focus:border-[#9a5f2a] focus:ring-0">{{ old('bio') }}</textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <input id="nbi_clearance_number" name="nbi_clearance_number" type="text" value="{{ old('nbi_clearance_number') }}" required class="w-full rounded-lg border border-[#d8c9b5] bg-[#f4efe6] px-3 py-2.5 text-sm text-[#2f241a] outline-none transition focus:border-[#9a5f2a] focus:ring-0">
                            </div>
                            <div>
                                ...
                                <input id="barangay_clearance_number" name="barangay_clearance_number" type="text" value="{{ old('barangay_clearance_number') }}" class="w-full rounded-lg border border-[#d8c9b5] bg-[#f4efe6] px-3 py-2.5 text-sm text-[#2f241a] outline-none transition focus:border-[#9a5f2a] focus:ring-0">
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="space-y-2 rounded-lg border border-[#d8c9b5] bg-[#f4efe6] p-3 text-sm text-[#6f5b46]">
                        <label class="flex items-start gap-2">
                            <input type="checkbox" name="terms_agreed" value="1" class="mt-1 h-4 w-4 rounded border-[#c8b49b] text-[#8b4e1c] focus:ring-[#8b4e1c]/30" {{ old('terms_agreed') ? 'checked' : '' }} required>
                            <span>I agree to the Terms and Conditions and Privacy Policy.</span>
                        </label>
                        <label class="flex items-start gap-2">
                            <input type="checkbox" name="identity_consent" value="1" class="mt-1 h-4 w-4 rounded border-[#c8b49b] text-[#8b4e1c] focus:ring-[#8b4e1c]/30" {{ old('identity_consent') ? 'checked' : '' }} required>
                            <span>I consent to identity verification.</span>
                        </label>
                        @if ($isGuide)
                            <label class="flex items-start gap-2">
                                <input type="checkbox" name="pending_understood" value="1" class="mt-1 h-4 w-4 rounded border-[#c8b49b] text-[#8b4e1c] focus:ring-[#8b4e1c]/30" {{ old('pending_understood') ? 'checked' : '' }} required>
