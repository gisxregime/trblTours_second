...
@section('title', $guide->full_name . ' - Tour Guide Profile')
            ...
            <div class="flex flex-col sm:flex-row gap-6 items-start sm:items-center">
                ...
                <div class="flex-shrink-0">
                    @if ($guide->profile_photo_path)
                        <img src="{{ $guide->profile_photo_path }}" 
                             alt="{{ $guide->full_name }}"
                    ...
                    @else
                        <div class="h-32 w-32 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center border-4 border-emerald-200 shadow-lg">
                            <span class="text-3xl font-bold text-white">{{ substr($guide->full_name, 0, 1) }}</span>
                        </div>
                    @endif
                </div>
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 flex-wrap">
                        <h1 class="text-3xl font-bold text-gray-900">{{ $guide->full_name }}</h1>
                        @if ($guide->tourGuideProfile?->approved_by_admin ?? false)
                    ...
                    <div class="mt-3 flex flex-wrap gap-4 text-sm text-gray-600">
                        <div class="flex items-center gap-1">
                            ...
                            <span class="font-semibold text-gray-900">{{ number_format($stats['averageRating'], 1) }}</span>
                            <span>({{ $stats['totalReviews'] }} reviews)</span>
                        </div>
                        <div>✓ {{ $stats['totalToursCompleted'] }} tours completed</div>
                        <div class="flex items-center gap-1">
                            <svg class="w-4 h-4 text-emerald-600" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd"/>
                            </svg>
                            <span>{{ $guide->region ?? 'Philippines' }}</span>
                        </div>
                    </div>

                    <!-- Additional Info -->
                    <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
                        @if ($guide->tourGuideProfile?->years_of_experience)
                            <div>
                                <span class="text-gray-600">Experience:</span>
                                <p class="font-semibold text-gray-900">{{ $guide->tourGuideProfile->years_of_experience }} years</p>
                            </div>
                        @endif
                        
                        @if ($guide->tourGuideProfile?->phone_number)
                            <div>
                                <span class="text-gray-600">Contact:</span>
                                <p class="font-semibold text-gray-900">{{ $guide->tourGuideProfile->phone_number }}</p>
                            </div>
                        @endif

                        <div>
                            <span class="text-gray-600">Specialty:</span>
                            <p class="font-semibold text-gray-900">{{ $guide->specialty ?? 'General Tours' }}</p>
                        </div>
                    </div>

                    <!-- About Section -->
                    @if ($guide->bio || $guide->tourGuideProfile?->bio)
                        <div class="mt-4 max-w-2xl">
                            <p class="text-gray-700 text-sm leading-relaxed">
                                {{ $guide->bio ?? $guide->tourGuideProfile?->bio ?? 'Passionate guide ready to show you the best experiences!' }}
                            </p>
                        </div>
                    @endif
                    ...
                    <div class="mt-4 flex flex-wrap gap-3">
                        <a href="https://wa.me/63{{ substr($guide->tourGuideProfile?->phone_number ?? '', 1) }}" 
                           target="_blank"
                    ...
                    @foreach ($tours as $tour)
                        <div class="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden">
                            ...
                            <div class="relative h-48 bg-gradient-to-br from-emerald-200 to-teal-200 overflow-hidden group">
                                @if ($tour->featured_image || $tour->image_url)
                                    <img src="{{ $tour->featured_image ?? $tour->image_url }}" 
                                         alt="{{ $tour->name }}"
                                         class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
                                @else
                                    <div class="w-full h-full flex items-center justify-center">
                            ...
                            <div class="p-5">
                                ...
                                <!-- Duration -->
                                @if ($tour->duration || $tour->duration_label)
                                    <div class="flex items-center gap-2 text-sm text-gray-600 mb-3">
                                        ...
                                        <span>{{ $tour->duration ?? $tour->duration_label ?? '1 day' }}</span>
                                    </div>
                                @endif

                                <!-- Description -->
                                <p class="text-sm text-gray-600 mb-4 line-clamp-2">
                                    {{ $tour->short_description ?? $tour->summary ?? 'Amazing tour experience waiting for you!' }}
                                </p>
                                ...
                                @if ($tour->min_guests || $tour->max_guests)
                                    <div class="flex items-center gap-2 text-sm text-gray-600 mb-4 pb-4 border-b border-gray-100">
                                        ...
                                        <span>
                                            {{ $tour->min_guests ?? 1 }} - {{ $tour->max_guests ?? 10 }} guests
                                        </span>
                                    </div>
                                @endif

                                <!-- Book Button -->
                                <button onclick="bookTour('{{ $tour->name ?? $tour->title }}')" 
